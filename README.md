...
<p align="center">
  Turn a DXF drawing of mounting holes into a wired
...
xModelGen reads a DXF file, finds the round holes in it (where the LEDs/pixels
go), works out the order to wire them, and exports an xLights `.xmodel` you can
import directly.

1. **Open a DXF** – holes are detected from circles, arcs, circle‑like
   polylines, and loops of connected line segments. Block references
   (`INSERT`s) are expanded to their real positions.
2. **Set the hole size** – the target diameter (mm) is configurable; only holes
   within ±1 mm of it are picked up. File units (`$INSUNITS`) are honoured.
3. **Pick a start node** – click (or click‑and‑drag) a hole in the view; it
   turns green.
4. **Auto Wire** – a recursive search finds an order that visits every hole
   within the wire gap, skipping and backtracking as needed. Wired holes turn
   blue and show their number.
...
```
xmodel_gen <file.dxf> [holeDiameterMm] [wireGapMm] [out.xmodel]
```
